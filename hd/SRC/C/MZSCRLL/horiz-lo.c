...
//    horiz_wall_chunk2_src_y = 70 + (x % 32);
//    horiz_wall_chunk2_src_addr = sprite_screen_addr + (horiz_wall_chunk2_src_y * LINE_SIZE_BYTES);
    ...
    for (col = 0; col < COLS; col++) {
      ...
      // };
      // horiz lines
      old_horiz_xoffset = logbase_addr + (((oldx + col * COL_WIDTH_PX) / 32 ) * 32);
      horiz_xoffset = logbase_addr + (((x + col * COL_WIDTH_PX) / 32) * 32);
...
//      memcpy((void*)dest_addr+16, (void*)horiz_wall_chunk2_src_addr, 16);
   }
