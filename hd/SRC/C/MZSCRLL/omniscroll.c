...
word *make_maze(word width, word height) {
  word *maze_data = malloc(sizeof(word) * MAZE_HEIGHT * MAZE_WIDTH);
  for(byte col=0; col < MAZE_WIDTH; col++) {
    for(byte row=0; row < MAZE_WIDTH; row++) {
      byte roll = rand() & 0XF;
    }
  }
}
...
int main() {
  ...
  word *maze_data = make_maze(MAZE_WIDTH, MAZE_HEIGHT);  
