...
for (word s = 0; s < page->num_vwalls; s++) {
  ...
  inlined_vline_draw((void*)vsd.start_addr, (void*)vsd.end_addr, LINE_SIZE_BYTES, 0);
}
}
...
for (word s = 0; s < page->num_vwalls; s++) {
  ...
  inlined_vline_draw((void*)vsd.start_addr, (void*)vsd.end_addr, LINE_SIZE_BYTES, value);
}
}
